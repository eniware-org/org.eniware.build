...
APP_ARGS="-Dsn.home=${ENIWAREEDGE_HOME} -Dderby.system.home=${DB_DIR} -Deniwarenetwork.pidfile=${PID_FILE} -Djava.util.logging.config.file=${ENIWAREEDGE_HOME}/conf/jre-logging.properties"
...
START_CMD="${JAVA_HOME}/bin/java ${JVM_ARGS} ${APP_ARGS} -jar ${ENIWAREEDGE_HOME}/app/${EQUINOX_JAR} -configuration ${EQUINOX_CONF} -console ${EQUINOX_CONSOLE} -clean"
START_SLEEP=14
STOP_TRIES=5
