...
#
# This script is designed to work with the eniwaressh@.service template service unit.
...
#
# eniwaressh.sh -u eniware -h sshserver.example.com -p 8022 -r 18932 start
#
# Some assumptions are made for this to work:
#
#  *  sudo is available
#  *  `sudo systemctl start eniwaressh@${instance}.service` can be run without
#     requesting a password
#  *  `sudo systemctl stop eniwaressh@${instance}.service` can be run without
#     requesting a password
...
RPORT="17777"
VERBOSE=""
ENV_DIR="/home/eniware/var/ssh"
SERVICE_NAME="eniwaressh"
...
verify_ssh_key() {
	if [ ! -e "${PUBKEY}" ]; then
		...
		ssh-keygen -q -t ${KEYTYPE} -C "EniwareEdge" -f ${PUBKEY%.*} -N ''
	fi
}
