...
### END INIT INFO
# 
# SysV init script for the EniwareEdge daemon for Eclipse Equinox. Designed
...
# description: Control the EniwareEdge Equinox server
#
...
PID_FILE=${RAM_DIR}/eniwareedge.pid
APP_ARGS="-Dsn.home=${ENIWAREEDGE_HOME} -Dderby.system.home=${DB_DIR} -Djava.util.logging.config.file=${ENIWAREEDGE_HOME}/conf/jre-logging.properties"
...
DAEMON_NAME="EniwareEdge"
DAEMON_CMD="${DAEMON} --name=${DAEMON_NAME} --pidfile=${PID_FILE} --chdir=${ENIWAREEDGE_HOME} --output=daemon.info --respawn --acceptable=20 --attempts=5 --delay=10"
if [ -n "${RUNAS}" ]; then
...
if [ -d ${ENIWAREEDGE_HOME}/lib ]; then
	LD_PATH="${LD_PATH}:${ENIWAREEDGE_HOME}/lib"
fi
...
JAVA_CMD="${JAVA_HOME}/bin/java ${JVM_ARGS} ${APP_ARGS} -jar ${ENIWAREEDGE_HOME}/app/${EQUINOX_JAR} -configuration ${EQUINOX_CONF} -console ${EQUINOX_CONSOLE} -clean"
...
setup_ini () {
	if [ ! -e "${EQUINOX_CONF}/config.ini" ]; then
		if [ -z "${RUNAS}" ]; then
			cp ${ENIWAREEDGE_HOME}/conf/config.ini ${EQUINOX_CONF}
		else
			su - $RUNAS -c "cp ${ENIWAREEDGE_HOME}/conf/config.ini ${EQUINOX_CONF}"
		fi
	fi
}
...
do_start () {
	...
	if ! ${DAEMON_CMD} --running; then
		...
		echo "EniwareEdge is already running (`cat $PID_FILE`)"
	fi
}
...
do_stop () {
	if ${DAEMON_CMD} --running; then
		echo -n "Stopping EniwareEdge `cat $PID_FILE`... "
...
do_status () {
 	if ${DAEMON_CMD} --running; then
 		echo "EniwareEdge is running (PID `cat $PID_FILE`)"
 	else
 		echo "EniwareEdge does not appear to be running."
 	fi
}
