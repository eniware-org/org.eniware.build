...
# Utilities for managing the EniwareEdge server
...
setup_ini () {
	if [ ! -e "${EQUINOX_CONF}/config.ini" -a -e "${ENIWAREEDGE_HOME}/conf/config.ini" ]; then
		if [ -z "${RUNAS}" ]; then
...
case $1 in
	...
	stop)
		exit_status=`systemctl show eniwareedge.service --no-pager |grep ExecMainStatus |cut -d= -f2`
